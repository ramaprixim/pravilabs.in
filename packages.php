<?php include("header.php") ?>
<style>
   .table>:not(caption)>*>*{
      padding: 0.5rem 1rem;
   }
</style>
         <!-- breadcrumb-area-end -->
         <!-- breadcrumb-area -->
         <section class="breadcrumb__area pt-100 pb-120 breadcrumb__overlay" data-background="assets/img/banner/breadcrumb-01.jpg">
            <div class="container">
               <div class="row align-items-center">
                  <div class="col-xl-7 col-lg-12 col-md-12 col-12">
                     <div class="tp-breadcrumb">
                        <h2 class="tp-breadcrumb__title">Packages</h2>
                     </div>
                  </div>
                  <div class="col-xl-5 col-lg-12 col-md-12 col-12">
                     <div class="tp-breadcrumb__link text-xl-end">
                        <span>Bioxlab : <a href="team-details.html"> Packages</span>
                     </div>
                  </div>
               </div>
            </div>
         </section>

<section style="padding-top: 100px;">
   <div class="container">
      <div class="row">
         <div class="col-lg-3 d-flex justify-content-center">
            <div class="plan">
               <div class="inner">
                  <span class="pricing">
                     <span>
                        59% Off
                     </span>
                  </span>
                  <p class="title">Full Body Checkup Package - <br> Basic</p>
                  <p class="info">Includes: 69 Parameters</p>
                  <ul class="features">
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>CBC</span>
                     </li>
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>BLOOD SUGAR RANDOM</span>
                     </li>
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>LFT</span>
                     </li>
                     
                  </ul>
                  <div class="row">
                     <div class="col-lg-6">
                        <button class="card-button1 mb-4">know More</button>
                     </div>
                     <div class="col-lg-6 d-flex justify-content-end align-items-center">
                        <h4 class="price-text">---</h4>  
                     </div>
                  </div>
                 
                  <div class="action">
                  <a class="card-button" href="#">
                     Book Now
                  </a>
                  <a class="card-button " href="#">
                    Get a Call Back
                  </a>
                  </div>
               </div>
            </div>
         </div>
         <div class="col-lg-3 d-flex justify-content-center">
			<div class="plan">
				<div class="inner">
				   <span class="pricing">
					  <span>
						 59% Off
					  </span>
				   </span>
				   <p class="title">Full Body Checkup Package -  <br> Advance</p>
				   <p class="info">Includes: 81 Parameters</p>
				   <ul class="features">
					  <li>
						 <span class="icon">
							<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							   <path d="M0 0h24v24H0z" fill="none"></path>
							   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
							</svg>
						 </span>
						 <span>CBC</span>
					  </li>
					  <li>
						 <span class="icon">
							<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							   <path d="M0 0h24v24H0z" fill="none"></path>
							   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
							</svg>
						 </span>
						 <span>BLOOD SUGAR</span>
					  </li>
					  <li>
						 <span class="icon">
							<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							   <path d="M0 0h24v24H0z" fill="none"></path>
							   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
							</svg>
						 </span>
						 <span>LFT</span>
					  </li>
					  
				   </ul>
				   <div class="row">
					  <div class="col-lg-6">
						 <button class="card-button1 mb-4">know More</button>
					  </div>
					  <div class="col-lg-6 d-flex justify-content-end align-items-center">
						 <h4 class="price-text">---</h4>  
					  </div>
				   </div>
				  
				   <div class="action">
				   <a class="card-button" href="#">
					  Book Now
				   </a>
				   <a class="card-button " href="#">
					 Get a Call Back
				   </a>
				   </div>
				</div>
			 </div>
            </div>
            <div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Full Body Checkup Package - <br>Exclusive </p>
					   <p class="info">Includes: 82 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>BLOOD SUGAR RANDOM</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LFT</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>
            <div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Full Body Checkup Package - <br> Comprehensive</p>
					   <p class="info">Includes: 85 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>BLOOD SUGAR FASTING</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HBA1c</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>
			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Obesity Panel-Basic <br> <br></p>
					   <p class="info">Includes: 13 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LIPID PROFILE</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>THYROID PROFILE</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>BLOOD SUGAR RANDOM</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>
			<div class="col-lg-3 d-flex justify-content-center">
            <div class="plan">
               <div class="inner">
                  <span class="pricing">
                     <span>
                        59% Off
                     </span>
                  </span>
                  <p class="title">Obesity Panel- <br>Advance</p>
                  <p class="info">Includes: 16 Parameters</p>
                  <ul class="features">
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>LIPID PROFILE</span>
                     </li>
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>THYROID PROFILE</span>
                     </li>
                     <li>
                        <span class="icon">
                           <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 0h24v24H0z" fill="none"></path>
                              <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
                           </svg>
                        </span>
                        <span>FT3</span>
                     </li>
                     
                  </ul>
                  <div class="row">
                     <div class="col-lg-6">
                        <button class="card-button1 mb-4">know More</button>
                     </div>
                     <div class="col-lg-6 d-flex justify-content-end align-items-center">
                        <h4 class="price-text">₹5899.00</h4>  
                     </div>
                  </div>
                  <div class="action">
                  <a class="card-button" href="#">
                     Book Now
                  </a>
                  <a class="card-button " href="#">
                    Get a Call Back
                  </a>
                  </div>
               </div>
            </div>
            </div>
			
			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fever Panel-Basic <br> <br></p>
					   <p class="info">Includes: 39 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>ESR</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>MALARIA P/S</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fever Panel-Advance <br> <br></p>
					   <p class="info">Includes: 42 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>ESR</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>MALARIA P/S</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fever Panel-Exclusive <br> <br></p>
					   <p class="info">Includes: 46 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>ESR</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>MALARIA P/S</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fever Panel-Comprehensive<br> <br></p>
					   <p class="info">Includes: 60 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>ESR</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>MALARIA P/S</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infections Profile <br> <br></p>
					   <p class="info">Includes: 5 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HIV I & II Quali</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HBsAg Quali</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HCV Quali</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infertility Female Profile-Basic <br> <br></p>
					   <p class="info">Includes: 3 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FSH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>PRL</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infertility Female Profile-<br> Advance</p>
					   <p class="info">Includes: 5 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FSH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>PRL</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">1199.00</h4>  
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infertility Female Profile-<br>Exclusive</p>
					   <p class="info">Includes: 9 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>AFP</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>ASAB</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>bHCG</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Covid Severity Profile<br> <br></p>
					   <p class="info">Includes: 35 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg> 
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <br> <br> <br>
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Covid Recovery Panel<br><br></p>
					   <p class="info">Includes: 65 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <br> <br> <br>
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infertility Female Profile-<br>Comprehensive</p>
					   <p class="info">Includes: 9 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FSH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>PRL</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Infertility Male Profile-1<br> <br></p>
					   <p class="info">Includes: 4 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FSH (Follicle Stimulating)</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LH (Luteinising Hormone)</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Testo Total</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Anemia Profile-Basic<br><br></p>
					   <p class="info">Includes: 33 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Ferritin</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Folic Acid</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Iron</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Anemia Profile-Advance<br><br></p>
					   <p class="info">Includes: 35 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Ferritin</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Folic Acid</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Vit B12</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">STD Detection Package<br><br></p>
					   <p class="info">Includes: 5 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HIV</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HBsAg</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HCV</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Thyroid Panel Free<br><br></p>
					   <p class="info">Includes: 3 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FT3</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FT4</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>TSH</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Female Hormone Profile With<br>Doctor Consultation</p>
					   <p class="info">Includes: 10 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>FSH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LH</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Prolactin (pooled)</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Arthritis Profile (For Joints Pain)<br><br></p>
					   <p class="info">Includes: 39 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CBC with ESR</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>CRP</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>RA Factor</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Thyroid Panel<br><br></p>
					   <p class="info">Includes: 3 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>T3</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>T4</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>TSH</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fitness Profile-Basic<br><br></p>
					   <p class="info">Includes: 14 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Hemoglobin</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Blood Glucose</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Cholesterol</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Fitness Profile-Advance<br><br></p>
					   <p class="info">Includes: 18 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Hemoglobin</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Blood Glucose</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Cholesterol</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Lipid Profile With Doctor<br> Consultation</p>
					   <p class="info">Includes: 8 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>LDL Cholesterol</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Total Cholesterol</span>
						  </li>
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>HDL Cholesterol</span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>
			
			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Cardiac Risk Markers With<br> Doctor Consultation</p>
					   <p class="info">Includes: 3 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Female Cancer Screening Test <br>With Doctor Consultation</p>
					   <p class="info">Includes: 6 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Food Intolerance Test - (210+<br> Foods)</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Male Cancer Screening With <br> Doctor Consultation</p>
					   <p class="info">Includes: 6 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Basic Diabetic Screening With  <br>Doctor Consultation</p>
					   <p class="info">Includes: 4 Parameters</p>
					   <ul class="features">
						  <li>
							  <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>  -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <br>
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Comprehensive Female<br> Hormone Profile</p>
					   <p class="info">Includes: 6 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> 
							 <span>Female infertility profile</span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">- </h4>  
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Arthritis Profile With Doctor<br> Consultation</p>
					   <p class="info">Includes: 25 Parameters</p>
					   <ul class="features">
						  <li>
							 <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span>
							 <span>Arthritis Profile</span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Pelvic Cancer Screening for<br> Women</p>
					   <p class="info">Includes: 6 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <br>
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Immunity Check - Basic<br><br></p>
					   <p class="info">Includes: 7 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Brave Heart Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Allergy Screening Package -<br>  Basic</p>
					   <p class="info">Includes: 0 Parameters	</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Well Women Checkup Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Menopause Wellness Packege<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Pancreatic Acute Profile<br> Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Dialysis Profile-3 Health<br>  Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Hypertension Profile-2 Checkup<br>Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Hepatitis Profile-2 Checkup<br>Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Hirsutism profile-1 Checkup <br>Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Thalassemia Profile Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Comprehensive Male Health<br> Checkup Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Hemoglobin Check Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Kids Health Checkup Plan<br>Package</p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Coagulation Profile Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">GDIC Fever Panel-2 Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">CNC Covid Care Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">Torch IgG and IgM Profile<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">CNC Champion Health Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

			<div class="col-lg-3 d-flex justify-content-center">
				<div class="plan">
					<div class="inner">
					   <span class="pricing">
						  <span>
							 59% Off
						  </span>
					   </span>
					   <p class="title">CNC Advance Health Package<br><br></p>
					   <p class="info">Includes: 0 Parameters</p>
					   <ul class="features">
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 <span></span>
						  </li>
						  <li>
							 <!-- <span class="icon">
								<svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
								   <path d="M0 0h24v24H0z" fill="none"></path>
								   <path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path>
								</svg>
							 </span> -->
							 
							 <span></span>
						  </li>
						  
					   </ul>
					   <div class="row">
						  <div class="col-lg-6">
							 <button class="card-button1 mb-4">know More</button>
						  </div>
						  <div class="col-lg-6 d-flex justify-content-end align-items-center">
							 <h4 class="price-text">---</h4>   
						  </div>
					   </div>
					  
					   <div class="action">
					   <a class="card-button" href="#">
						  Book Now
					   </a>
					   <a class="card-button " href="#">
						 Get a Call Back
					   </a>
					   </div>
					</div>
				 </div>
            </div>

      </div>
   </div> 
</section>
<section style="padding-top: 100px;">
   <div class="container">
      <div class="row">

            <h2>Full Body Checkup At Navi Mumbai</h2>
            <p>A complete physical examination that diagnoses and carefully evaluates the health status of a healthy body is known as a comprehensive, complete physical examination. It helps in comprehensive assessment of all body systems and organs. It consists of various tests that provide information about a person's current health status. It includes more than 60 tests to determine the health status of every body organ, including the thyroid, teeth, eyes, bones, liver, kidneys and more.</p>


      </div>
   </div>
   <div class="container pt-40">
<div class="card" style="padding: 24px 20px;">
   <h2>Full Body Checkup Test List </h2>
   <p>A full body checkup involves a variety of medical tests and examinations to assess a person's overall health and wellbeing. Navi Mumbai, there are numerous healthcare providers and hospitals that offer full body checkup services. Here are some of the common services that pravilabs in Navi Mumbai list for a full body checkup package test:</p>
   <ul>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; CBC and HPLC test</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; PFT Test (Pulmonary Function Test)</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Cholesterol and lipid profile</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Chest X-rays, ECG tests and 2D echocardiography are all part of the cardiac system.</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Check your vitamins</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Abdominal Ultrasonography: Ultrasonography of abdomen and pelvis</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; TSH, T3 and T4 thyroid function tests</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Kidney function test (KFT): urea, uric acid and serum creatinine</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; SGPT, SGOT, and Serum Bilirubin: Liver Function Test (LFT)</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Diabetes screening: Fasting blood sugar and PP Hepatitis B test</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp;  Calcium scoring in CT</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Total body fat percentage</li>
      <li><i class="fa-solid fa-circle-dot"></i>&nbsp; Check your eyes</li>
   </ul>
   <br>
   <p>In addition, some full body checkup packages in Navi Mumbai may also include additional tests such as Pap smear for women, PSA test for men and vitamin D and B12 tests. It is important to consult with a healthcare professional to understand what tests are needed based on individual health concerns and medical history. A full body checkup can be an effective way to detect health problems early and prevent or manage chronic diseases, making it an important aspect of overall health and wellness.</p>
</div>
   </div>

</section>
<section style="padding: 100px 0px;">
   <div class="container">
      <div class="row">
         <div class="col-lg-3">

         </div>
         <div class="col-lg-3"></div>
      </div>
      <div>
   </div>
   </div>
   <div class="container">
      <div class="row align-items-center">
         <div class="col-lg-12">
            <div class="tp-section mb-20">
               <br>
               <h3 class="tp-section__title mb-60 text-center">Here are the Benefits of a full body checkup</h3>
            </div>
         </div>
      </div>
      <div class="row">
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Early detection of health issues</h5>
                              <p>A full body checkup allows doctors to screen for various diseases and conditions at an early stage. This helps in early detection and timely treatment, leading to better health outcomes.</p>
                          <br>
                          <br>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Comprehensive assessment</h5>
                              <p>The checkup involves a series of tests and examinations that cover all major systems of the body. This includes laboratory tests like blood work, urine analysis, and imaging tests like X-rays or ultrasounds. It provides a holistic view of your overall health.</p>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Personalized health advice</h5>
                              <p>Based on the results of the checkup, doctors can provide personalized health advice and recommendations. This may include lifestyle modifications, diet changes, exercise routines, and specific preventive measures to address any identified health risks.</p>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
      <div class="row">
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Peace of mind</h5>
                              <p>Knowing that you have undergone a thorough checkup and received a clean bill of health can relieve anxiety and provide peace of mind. It allows you to continue with your daily activities without worrying about any underlying health issues.</p>
                          <br>
                          <br>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Preventive measures</h5>
                              <p>A full body checkup can identify risk factors and warning signs for potential future health problems. This gives you the opportunity to take preventive measures before the condition worsens or becomes irreversible. For example, if high cholesterol levels are detected, you can start making lifestyle changes to prevent heart disease.</p>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
         <div class="col-xxl-4 col-lg-6 col-md-6">
            <div class="blogthumb mb-30 wow fadeInUp" data-wow-delay=".3s" data-background="assets/img/blog/blog-thumb-07.jpg" style="background-image: url(); visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">
               <div class="blogitem" style="background: #dcecf9;">
                  <div class="fix inner-blog-wrap">
                     <div class="blogitem__avata-part">
                        <div class="blogitem__avata">
                           
                           <div class="blogitem__avata-content">
                              <div class="" style="padding: 10px 10px;">
                                 <img src="assets/img/icon/medical-team.png" alt="">
                                     </div>
                              <h5 class="blogitem__title mb-20">Health awareness and education</h5>
                              <p>Through the checkup process, you gain valuable knowledge about your own health status. Doctors can explain any health concerns or abnormalities found during the checkup, which helps you understand your body and take charge of your well-being.</p>
                          <br>
                           </div>
                        </div>
                        <div class="blogitem__medi">
                          
                        </div>
                     </div>
                     <div class="blogitem__title-area p-relative">
                        <div class="blog-item__date-info mb-15">
                          
                        </div>
                       
                        <div class="tp-blog__btn blog-bg-btn">
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>   
</section>
<section style="padding: 150px 0px;">
   <div class="container">
      <table class="table">
         <thead class="thead-light">
             <tr>
                 <th scope="col">Package Name</th>
                 <th scope="col">Amount</th>
             </tr>
         </thead>
         <tbody>
                                         <tr>
                     <td>Full Body Checkup Package - Basic</td>
                     <td>₹ 2,000.00</td>
                 </tr>
                                         <tr>
                     <td>Full Body Checkup Package - Advance</td>
                     <td>₹ 3,333.00</td>
                 </tr>
                                         <tr>
                     <td>Full Body Checkup Package - Exclusive</td>
                     <td>₹ 4,659.00</td>
                 </tr>
                                         <tr>
                     <td>Full Body Checkup Package - Comprehensive</td>
                     <td>₹ 5,959.00</td>
                 </tr>
                                         <tr>
                     <td>Obesity Panel-Basic</td>
                     <td>₹ 1,719.00</td>
                 </tr>
                                         <tr>
                     <td>Obesity Panel-Advance</td>
                     <td>₹ 6,319.00</td>
                 </tr>
                                         <tr>
                     <td>Fever Panel-Basic</td>
                     <td>₹ 699.00</td>
                 </tr>
                                         <tr>
                     <td>Fever Panel-Advance</td>
                     <td>₹ 899.00</td>
                 </tr>
                                         <tr>
                     <td>Fever Panel-Exclusive</td>
                     <td>₹ 1,299.00</td>
                 </tr>
                                         <tr>
                     <td>Fever Panel-Comprehensive</td>
                     <td>₹ 2,499.00</td>
                 </tr>
                                         <tr>
                     <td>Infections Profile</td>
                     <td>₹ 1,199.00</td>
                 </tr>
                                         <tr>
                     <td>Infertility Female Profile-Basic</td>
                     <td>₹ 1,099.00</td>
                 </tr>
                                         <tr>
                     <td>Infertility Female Profile-Advance</td>
                     <td>₹ 1,499.00</td>
                 </tr>
                                         <tr>
                     <td>Infertility Female Profile-Exclusive</td>
                     <td>₹ 4,099.00</td>
                 </tr>
                                         <tr>
                     <td>Covid Severity Profile</td>
                     <td>₹ 3,999.00</td>
                 </tr>
                                         <tr>
                     <td>Covid Recovery Panel</td>
                     <td>₹ 4,499.00</td>
                 </tr>
                                         <tr>
                     <td>Infertility Female Profile-Comprehensive</td>
                     <td>₹ 2,499.00</td>
                 </tr>
                                         <tr>
                     <td>Infertility Male Profile-1</td>
                     <td>₹ 1,799.00</td>
                 </tr>
                                         <tr>
                     <td>Anemia Profile-Basic</td>
                     <td>₹ 2,499.00</td>
                 </tr>
                                         <tr>
                     <td>Anemia Profile-Advance</td>
                     <td>₹ 2,499.00</td>
                 </tr>
                                         <tr>
                     <td>STD Detection Package</td>
                     <td>₹ 1,798.00</td>
                 </tr>
                                         <tr>
                     <td>Thyroid Panel Free</td>
                     <td>₹ 899.00</td>
                 </tr>
                                         <tr>
                     <td>Female Hormone Profile With Doctor Consultation</td>
                     <td>₹ 5,000.00</td>
                 </tr>
                                         <tr>
                     <td>Arthritis Profile (For Joints Pain)</td>
                     <td>₹ 2,999.00</td>
                 </tr>
                                         <tr>
                     <td>Thyroid Panel</td>
                     <td>₹ 799.00</td>
                 </tr>
                                         <tr>
                     <td>Fitness Profile-Basic</td>
                     <td>₹ 1,999.00</td>
                 </tr>
                                         <tr>
                     <td>Fitness Profile-Advance</td>
                     <td>₹ 2,999.00</td>
                 </tr>
                                         <tr>
                     <td>Lipid Profile With Doctor Consultation</td>
                     <td>₹ 699.00</td>
                 </tr>
                                         <tr>
                     <td>Cardiac Risk Markers With Doctor Consultation</td>
                     <td>₹ 4,000.00</td>
                 </tr>
                                         <tr>
                     <td>Female Cancer Screening Test With Doctor Consultation</td>
                     <td>₹ 8,500.00</td>
                 </tr>
                                         <tr>
                     <td>Food Intolerance Test - (210+ Foods)</td>
                     <td>₹ 18,000.00</td>
                 </tr>
                                         <tr>
                     <td>Male Cancer Screening With Doctor Consultation</td>
                     <td>₹ 8,500.00</td>
                 </tr>
                                         <tr>
                     <td>Basic Diabetic Screening With Doctor Consultation</td>
                     <td>₹ 2,850.00</td>
                 </tr>
                                         <tr>
                     <td>Comprehensive Female Hormone Profile</td>
                     <td>₹ 3,998.00</td>
                 </tr>
                                         <tr>
                     <td>Arthritis Profile With Doctor Consultation</td>
                     <td>₹ 8,599.00</td>
                 </tr>
                                         <tr>
                     <td>Pelvic Cancer Screening for Women</td>
                     <td>₹ 6,500.00</td>
                 </tr>
                                         <tr>
                     <td>Immunity Check - Basic</td>
                     <td>₹ 4,500.00</td>
                 </tr>
                                         <tr>
                     <td>Brave Heart Package</td>
                     <td>₹ 6,000.00</td>
                 </tr>
                                         <tr>
                     <td>Allergy Screening Package - Basic</td>
                     <td>₹ 1,500.00</td>
                 </tr>
                                         <tr>
                     <td>Well Women Checkup Package</td>
                     <td>₹ 1,080.00</td>
                 </tr>
                                         <tr>
                     <td>Menopause Wellness Packege</td>
                     <td>₹ 800.00</td>
                 </tr>
                                         <tr>
                     <td>Pancreatic Acute Profile Package</td>
                     <td>₹ 1,740.00</td>
                 </tr>
                                         <tr>
                     <td>Dialysis Profile-3 Health Package</td>
                     <td>₹ 2,360.00</td>
                 </tr>
                                         <tr>
                     <td>Hypertension Profile-2 Checkup Package</td>
                     <td>₹ 1,670.00</td>
                 </tr>
                                         <tr>
                     <td>Hepatitis Profile-2 Checkup Package</td>
                     <td>₹ 9,700.00</td>
                 </tr>
                                         <tr>
                     <td>Hirsutism profile-1 Checkup Package</td>
                     <td>₹ 4,960.00</td>
                 </tr>
                                         <tr>
                     <td>Preoperative Profile Health Package</td>
                     <td>₹ 2,120.00</td>
                 </tr>
                                         <tr>
                     <td>Thalassemia Profile Package</td>
                     <td>₹ 2,230.00</td>
                 </tr>
                                         <tr>
                     <td>Executive Cardiac and Health Checkup Screening Package</td>
                     <td>₹ 4,830.00</td>
                 </tr>
                                         <tr>
                     <td>Comprehensive Female Health Checkup Package</td>
                     <td>₹ 1,820.00</td>
                 </tr>
                                         <tr>
                     <td>Comprehensive Male Health Checkup Package</td>
                     <td>₹ 1,530.00</td>
                 </tr>
                                         <tr>
                     <td>Hemoglobin Check Package</td>
                     <td>₹ 3,320.00</td>
                 </tr>
                                         <tr>
                     <td>Kids Health Checkup Plan Package</td>
                     <td>₹ 1,544.00</td>
                 </tr>
                                         <tr>
                     <td>Coagulation Profile Package</td>
                     <td>₹ 1,780.00</td>
                 </tr>
                                         <tr>
                     <td>GDIC Fever Panel-2 Package</td>
                     <td>₹ 3,210.00</td>
                 </tr>
                                         <tr>
                     <td>CNC Covid Care Package</td>
                     <td>₹ 6,700.00</td>
                 </tr>
                                         <tr>
                     <td>Torch IgG and IgM Profile</td>
                     <td>₹ 4,940.00</td>
                 </tr>
                                         <tr>
                     <td>CNC Champion Health Package</td>
                     <td>₹ 5,469.00</td>
                 </tr>
                                         <tr>
                     <td>CNC Advance Health Package</td>
                     <td>₹ 5,781.00</td>
                 </tr>
                                 </tbody>
     </table>

   </div>

   
</section>

<section class="faq-area pt-125 pb-100">
   <div class="container">
      <div class="row text-center">
         <div class="col-lg-12 col-md-12 col-12">
            <div class="tp-section">
               <span class="tp-section__sub-title left-line right-line mb-20">Faq</span>
               <h3 class="tp-section__title mb-70">General Question</h3>
            </div>
         </div>
      </div>
      <div class="row">
         <div class="col-lg-12">
            <div class="faq-accordion">
               <div class="accordion" id="accordionExample">
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingOne">
                      <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">Who needed a Full Body Checkup?  </button>
                    </h2>
                    <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Anyone over the age of 25 who has not had a full body checkup delhi in the last 6-12 months and is interested in learning more about their health.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingTwo">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">How to Prepare for a Full Body Checkup? </button>
                    </h2>
                    <div id="collapseTwo" class="accordion-collapse collapse" aria-labelledby="headingTwo" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Make certain that you get enough sleep. Foods that are oily, hot, or salty should be avoided. Dinner should always include light fare. If you must fast, do so for 8-10 hours. Avoid drinking alcoholic beverages.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingThree">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">What exactly is a routine health checkup? </button>
                    </h2>
                    <div id="collapseThree" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>A routine health checkup consists of a battery of medical tests designed to assess your overall health. A health checkup will typically include blood sugar, cholesterol, kidney, liver function, blood work, and thyroid tests. If any of the test results are abnormal, the doctor may recommend additional tests.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingFour">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFour" aria-expanded="false" aria-controls="collapseFour">What is the purpose of a Full Body Health Checkup? </button>
                    </h2>
                    <div id="collapseFour" class="accordion-collapse collapse" aria-labelledby="headingFour" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Full Body Health Checkups are important because they help detect early signs of illness, allowing the problem to be reversed or cured. Full Body Health Checkups are indicators of how your lifestyle choices affect your health, and they provide you with the information you need to make changes to live a healthier, more fulfilling life.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingFive">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFive" aria-expanded="false" aria-controls="collapseFive">Why is a Full Body Checkup necessary? </button>
                    </h2>
                    <div id="collapseFive" class="accordion-collapse collapse" aria-labelledby="headingFive" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>A regular body check-up can assist doctors in diagnosing a disease before it poses a high risk. Reduces the risk of treatment complications - When you are diagnosed with a health condition at an early stage, the complexity and risk involved are less than when you are diagnosed at a late stage.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingSix">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSix" aria-expanded="false" aria-controls="collapseSix">What exactly is a yearly full body checkup? </button>
                    </h2>
                    <div id="collapseSix" class="accordion-collapse collapse" aria-labelledby="headingSix" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Anyone over the age of 25 who has not had a full body checkup in the last 6-12 months and is interested in learning more about their health.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingSeven">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSeven" aria-expanded="false" aria-controls="collapseSeven">What is included in a full Body checkup? </button>
                    </h2>
                    <div id="collapseSeven" class="accordion-collapse collapse" aria-labelledby="headingSeven" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Here is the list that includes:</p>
                        <ol><li><p>Vitals Checkup:</p><p>Your blood pressure, respiratory, temperature, and heart rate will be taken. Taking these vital checkups will help in identifying the actual problem.</p></li><li><p>Physical checkup:</p><p>Your physical checkups will also be done, some of the general checkups will include the heart (cardiovascular system), tummy (abdominal system), lungs (respiratory system), etc. These are the part of the body that manages the function of your body.</p></li><li><p>Medical Tests:</p><p>Medical Tests include liver functioning tests, kidney function tests, diabetes checks, thyroid function checks, and several other medical tests based on the symptoms.</p></li></ol></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                    <h2 class="accordion-header" id="headingEight">
                      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEight" aria-expanded="false" aria-controls="collapseEight">How frequently should you perform a full body checkup? </button>
                    </h2>
                    <div id="collapseEight" class="accordion-collapse collapse" aria-labelledby="headingEight" data-bs-parent="#accordionExample">
                      <div class="accordion-content"><p>Most people only get a full body checkup delhi when they have a minor or major medical emergency. However, a full body checkup delhi should be performed at least once every 6-12 months to ensure that you are immune to and avoid any future medical issues.</p></div>
                    </div>
                  </div>
                  <div class="accordion-items">
                     <h2 class="accordion-header" id="headingNine">
                       <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseNine" aria-expanded="false" aria-controls="collapseNine">What is the price of full body checkup?  </button>
                     </h2>
                     <div id="collapseNine" class="accordion-collapse collapse" aria-labelledby="headingNine" data-bs-parent="#accordionExample">
                       <div class="accordion-content"><p>Full body checkup price starts at ₹699. Full body checkup price can be different for different parameters or requirement of tests.</p></div>
                     </div>
                   </div>
                   <div class="accordion-items">
                     <h2 class="accordion-header" id="headingTen">
                       <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTen" aria-expanded="false" aria-controls="collapseTen">What is a full body checkup home collection package?  </button>
                     </h2>
                     <div id="collapseTen" class="accordion-collapse collapse" aria-labelledby="headingTen" data-bs-parent="#accordionExample">
                       <div class="accordion-content"><p>A full body checkup home collection package is a comprehensive health checkup package that includes a range of tests to assess the overall health and wellness of an individual. The tests are usually performed on blood and urine samples collected from the individual, and the package may also include a physical examination.</p></div>
                     </div>
                   </div>
                   <div class="accordion-items">
                     <h2 class="accordion-header" id="headingEleven">
                       <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEleven" aria-expanded="false" aria-controls="collapseEleven">Is fasting required for a full body checkup home collection package?   </button>
                     </h2>
                     <div id="collapseEleven" class="accordion-collapse collapse" aria-labelledby="headingEleven" data-bs-parent="#accordionExample">
                       <div class="accordion-content"><p>Fasting may be required for some tests included in a full body checkup home collection package, such as tests for blood sugar and cholesterol. It is important to follow the instructions provided by the health service provider regarding fasting requirements and other preparations for the tests.</p></div>
                     </div>
                   </div>
                   <div class="accordion-items">
                     <h2 class="accordion-header" id="headingTwelve">
                       <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwelve" aria-expanded="false" aria-controls="collapseTwelve">How long does it take to receive the results of a full body checkup home collection package?   </button>
                     </h2>
                     <div id="collapseTwelve" class="accordion-collapse collapse" aria-labelledby="headingTwelve" data-bs-parent="#accordionExample">
                       <div class="accordion-content"><p>The time it takes to receive the results of a full body checkup home collection package may vary depending on the provider and the specific tests included in the package. Some providers may offer same-day or next-day results, while others may take several days or weeks to process the samples and generate the results. It is recommended to ask the provider about the expected turnaround time for the results.</p></div>
                     </div>
                   </div>
                </div>
            </div>
         </div>
      </div>
   </div>
</section>
     <?php include("footer-2.php") ?>